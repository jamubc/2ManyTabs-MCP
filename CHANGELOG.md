...
### Added
- **Firefox support**: The extension now builds for Firefox (MV2) alongside Chrome (MV3) from a single source tree. Tab-group tools are unsupported on Firefox (no native `tabGroups` API) and fail with a clear per-tool error instead of breaking the extension.
...
### Changed
- **Extension rebuilt on WXT**: Migrated the extension from a flat `chrome.*` MV3-only tree to a [WXT](https://wxt.dev) project (`entrypoints/`, `lib/tab-ops.js`, `wxt.config.ts`) using the cross-browser `browser.*` API, so Chrome and Firefox now ship from one codebase instead of drifting into two. `native-host/` is unchanged and stays browser-agnostic; only `extension/` gained a build step.
